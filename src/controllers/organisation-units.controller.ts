import {Request, RestBindings, Response, get, param} from '@loopback/rest';
...
export class OrganisationUnitsController {
  ...
  async organisationUnits(@param.query.string('name') name = ''): Promise<any> {
    ...
    const rawOrganisationUnits = response.data.organisationUnits.filter(
